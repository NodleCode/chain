...
use runtime_main::{
	constants::*, wasm_binary_unwrap, AuthorityDiscoveryConfig, BabeConfig, BalancesConfig, FinancialMembershipConfig,
	GrandpaConfig, ImOnlineConfig, KnownCustomerMembershipConfig, RootMembershipConfig, SessionConfig, SessionKeys,
	SystemConfig, TechnicalMembershipConfig, ValidatorsSetConfig, VestingConfig, WnodlConfig,
...
pub fn testnet_genesis(
	...
	GenesisConfig {
		...
		balances: BalancesConfig {
			balances: endowed_accounts
				...
				.fold(vec![], |mut acc, (account, endowment)| {
					if acc.iter().any(|(who, _endowment)| who == &account) {
						// Increase endowment
						acc = acc
							.iter()
							.cloned()
		...
		vesting: VestingConfig { vesting: vested_grants },
