...
use nodle_chain_runtime::{Block, GenesisConfig, RuntimeApi};
...
type ConcreteBlock = nodle_chain_runtime::Block;
type ConcreteClient = Client<
    Backend<ConcreteBlock>,
    LocalCallExecutor<Backend<ConcreteBlock>, NativeExecutor<executor::Executor>>,
    ConcreteBlock,
    nodle_chain_runtime::RuntimeApi,
>;
...
pub fn new_light(config: NodeConfiguration) -> Result<impl AbstractService, ServiceError> {
    ...
    let service = ServiceBuilder::new_light::<Block, RuntimeApi, executor::Executor>(config)?
