...
pub mod pallet {
	...
	pub type Balances<T: Config> =
		StorageMap<_, Twox64Concat, T::AccountId, (BalanceOf<T>, BalanceOf<T>, BalanceOf<T>), ValueQuery>;
	...
	pub enum Event<T: Config> {
		/// Wrapping customer's fund is initiated \[account's address on Nodle chain, amount of Nodl
		/// fund, destination address on Ethereum main-net\]
		WrappingInitiated(T::AccountId, BalanceOf<T>, EthAddress),

		/// Wrapping Reserve fund is initiated \[account's address on Nodle chain, amount of Nodl
		/// fund, destination address on Ethereum main-net\]
		WrappingReserveInitiated(BalanceOf<T>, EthAddress),

		/// Wrapping customer's fund is settled \[account's address on Nodle chain, amount of Nodl
		/// fund settled, Transaction hash on Ethereum main-net\]
		WrappingSettled(T::AccountId, BalanceOf<T>, EthTxHash),

		/// Wrapping customer's fund is settled \[account's address on Nodle chain, amount of Nodl
		/// fund settled, destination address on Ethereum main-net, rejection code\]
		WrappingRejected(T::AccountId, BalanceOf<T>, EthAddress, RejectionCode),

		/// Wrapping Reserve fund is settled \[account's address on Nodle chain, amount of Nodl fund
		/// settled, Transaction hash on Ethereum main-net\]
		WrappingReserveSettled(BalanceOf<T>, EthTxHash),

		/// Wrapping customer's fund is settled \[account's address on Nodle chain, amount of Nodl
		/// fund settled, destination address on Ethereum main-net, reason\]
	...
	pub enum Error<T> {
		...
		FundNotWithinLimits,
		/// We do not expect this error ever happen. But if it happened we would not allow it to
		/// mess with the correctness of the storage.
		BalanceOverflow,
		/// The customer is not known, whitelisted for this operation.
		NotEligible,
		/// Settlement is only possible for an amount equal or smaller than an initiated wrapping
		/// for a known customer
		InvalidSettle,
		/// Reject is only possible for an amount equal or smaller than an initiated wrapping for a
		/// known customer
		InvalidReject,
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn initiate_wrapping(origin: OriginFor<T>, amount: BalanceOf<T>, eth_dest: EthAddress) -> DispatchResult {
			...
			ensure!(T::Currency::can_reserve(&who, amount), <Error<T>>::BalanceNotEnough);
		...
		pub fn initiate_wrapping_reserve_fund(
			...
			<TotalInitiated<T>>::put(total_initiated);
			<Balances<T>>::insert(reserve_account_id, (initiated, balances.1, balances.2));
		...
		pub fn settle(
			...
			if let Some(whitelisted_callers) = WhitelistedCallers::<T>::get() {
				...
				ensure!(T::KnownCustomers::contains(&customer), <Error<T>>::NotEligible);
			}
			#[cfg(not(feature = "runtime-benchmarks"))]
			ensure!(T::Oracles::contains(&who), <Error<T>>::NotEligible);
			#[cfg(not(feature = "runtime-benchmarks"))]
			...
			let settled_or_rejected = settled
				...
				.ok_or::<Error<T>>(Error::BalanceOverflow)?;
			// The amount of initiated wrapping should always be greater than or equal the sum of settled and
			// rejected
		...
		pub fn settle_reserve_fund(
			...
			<TotalSettled<T>>::put(total_settled);
			<Balances<T>>::insert(reserve_account_id.clone(), (balances.0, settled, balances.2));
		...
		pub fn reject_reserve_fund(
			...
			<TotalRejected<T>>::put(total_rejected);
			<Balances<T>>::insert(reserve_account_id.clone(), (balances.0, balances.1, rejected));
