...
fn known_customer_cannot_initiate_wrapping_zero_amount() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			Wnodl::initiate_wrapping(Origin::signed(KNOWN_CUSTOMERS[0]), 0, EthAddress::from(&[0u8; 20])),
...
fn keep_track_of_initiated_wnodl_per_customer() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Wnodl::total_initiated(), amount1 + amount2);
		assert_eq!(Wnodl::total_settled(), 0);
		assert_eq!(Wnodl::balances(&KNOWN_CUSTOMERS[0]), (amount1 + amount2, 0, 0));
	});
}
...
fn initiate_wrapping_generate_expected_event() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			last_event(),
			mock::Event::Wnodl(crate::Event::WrappingInitiated(KNOWN_CUSTOMERS[0], amount, eth_address))
		);
	});
}
...
fn trusted_oracle_can_reject() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			last_event(),
			mock::Event::Wnodl(crate::Event::WrappingRejected(
				KNOWN_CUSTOMERS[0],
				amount,
				eth_address,
				rejection_code
...
fn settling_les_than_initiated_is_ok() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Wnodl::balances(&KNOWN_CUSTOMERS[0]), (amount, amount - 1, 0));
	});
}
...
fn partly_settled_partly_rejected_works() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Wnodl::balances(&KNOWN_CUSTOMERS[0]), (amount, amount - 1, 1));
	});
}
...
fn rejecting_les_than_initiated_is_ok() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Wnodl::balances(&KNOWN_CUSTOMERS[0]), (amount, 0, amount - 1));
	});
}
...
fn root_can_settle_wrapping_reserve_fund() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Wnodl::settle_reserve_fund(Origin::root(), amount, eth_hash));
		assert_eq!(
			last_event(),
...
fn root_can_reject_wrapping_reserve_fund() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			last_event(),
			mock::Event::Wnodl(crate::Event::WrappingReserveRejected(amount, eth_address, reason))
		);
		assert_eq!(Wnodl::total_initiated(), amount);
		assert_eq!(Wnodl::total_settled(), 0);
		assert_eq!(Wnodl::total_rejected(), amount);
...
fn root_cannot_initiate_or_settle_or_reject_wrapping_from_reserve_fund_with_zero_amount() {
	new_test_ext().execute_with(|| {
		let amount = 0u64;
		assert_noop!(
			Wnodl::initiate_wrapping_reserve_fund(Origin::root(), amount, EthAddress::from(&[0u8; 20])),
			<Error<Test>>::UselessZero
		);
		assert_noop!(
...
fn non_root_cannot_settle_wrapping_reserve_fund() {
	new_test_ext().execute_with(|| {
		let amount = 43u64;
		assert_noop!(
			Wnodl::settle_reserve_fund(Origin::signed(KNOWN_CUSTOMERS[0]), amount, EthTxHash::from(&[0u8; 32]),),
			BadOrigin
		);
	});
}
...
fn root_is_not_limited_to_min_max_when_initiating_wrapping_reserve_fund() {
	new_test_ext().execute_with(|| {
		...
		let reserve_account_id = mock::Reserve::account_id();
		assert_eq!(Wnodl::balances(reserve_account_id), (amount1 + amount2, 0, 0));
		assert!(mock::Balances::free_balance(&reserve_account_id) == RESERVE_BALANCE - amount1 - amount2);
