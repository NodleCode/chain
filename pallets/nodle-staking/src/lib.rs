...
pub mod pallet {
		...
		> Nominator<AccountId, Balance>
	{
		pub fn new(validator: AccountId, amount: Balance) -> Self {
			Nominator {
				nominations: OrderedSet::from(vec![Bond { owner: validator, amount }]),
				total: amount,
			}
		}
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn join_validator_pool(
			...
			ensure!(bond >= T::MinValidatorPoolStake::get(), Error::<T>::ValidatorBondBelowMin);
			let mut validators = <ValidatorPool<T>>::get();
			ensure!(
				validators.insert(Bond { owner: acc.clone(), amount: bond }),
		...
		pub fn validator_bond_less(
			...
			ensure!(after >= T::MinValidatorPoolStake::get(), Error::<T>::ValidatorBondBelowMin);
		...
		pub fn nominate(
			...
			if let Some(mut nominator) = <NominatorState<T>>::get(&acc) {
				...
				let nomination = Bond { owner: acc.clone(), amount };
				...
				// first nomination
				ensure!(amount >= T::MinNominatorStake::get(), Error::<T>::NominatorBondBelowMin);
		...
		pub fn nominator_bond_more(
			...
			Self::deposit_event(Event::NominationIncreased(nominator, validator, before, after));
			Ok(().into())
		}
	...
	impl<T: Config> Default for GenesisConfig<T> {
		fn default() -> Self {
			Self { stakers: vec![], invulnerables: Default::default() }
		}
	}
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn update_validators_pool(validator: T::AccountId, total: BalanceOf<T>) {
			log!(trace, "[{:#?}]:[{:#?}] | Own[{:#?}]", function!(), line!(), validator);
			let mut validators = <ValidatorPool<T>>::get();
			validators.remove(&Bond::from_owner(validator.clone()));
			validators.insert(Bond { owner: validator, amount: total });
			<ValidatorPool<T>>::put(validators);
		}
		// ensure validator is active before calling
		...
		fn nominator_revokes_validator(
			...
			let remaining =
				nominator.rm_nomination(validator.clone()).ok_or(Error::<T>::NominationDNE)?;

			ensure!(remaining >= T::MinNominatorStake::get(), Error::<T>::NominatorBondBelowMin);
		...
		fn pay_stakers(next: SessionIndex) {
			log!(trace, "[{:#?}]:[{:#?}] - Sess-idx[{:#?}]", function!(), line!(), next);
			...
			for (val, pts) in <AwardedPts<T>>::drain_prefix(next) {
				...
				if state.nominators.is_empty() {
					...
					log!(trace, "[{:#?}]:[{:#?}] - L3 Solo Mode", function!(), line!());
				} else {
		...
		T: Config + pallet_authorship::Config + pallet_session::Config,
	{
		fn note_author(author: T::AccountId) {
			log!(trace, "[{:#?}]:[{:#?}] - Author[{:#?}]", function!(), line!(), author);
	...
	impl<T: Config> pallet_session::SessionManager<T::AccountId> for Pallet<T> {
		fn new_session(new_index: SessionIndex) -> Option<Vec<T::AccountId>> {
			log!(trace, "[{:#?}]:[{:#?}] - Sess-idx[{:#?}]", function!(), line!(), new_index);
		...
		fn start_session(start_index: SessionIndex) {
			log!(trace, "[{:#?}]:[{:#?}] - Sess-idx[{:#?}]", function!(), line!(), start_index);
			...
			<BondedSessions<T>>::mutate(|bonded| {
				...
				if start_index > bonding_duration {
					...
					let n_to_prune =
						bonded.iter().take_while(|&&session_idx| session_idx < first_kept).count();
		...
		fn end_session(end_index: SessionIndex) {
			log!(trace, "[{:#?}]:[{:#?}] - Sess-idx[{:#?}]", function!(), line!(), end_index);
		...
		O: Offence<Offender>,
	{
		fn report_offence(reporters: Vec<Reporter>, offence: O) -> Result<(), OffenceError> {
			...
			if bonded_sessions.first().filter(|start| offence_session >= **start).is_some() {
