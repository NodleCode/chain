...
pub(crate) fn compute_slash<T: Config>(
    ...
    // apply slash to validator.
    {
        ...
        if target_span == Some(spans.span_index()) {
            ...
            spans.end_span(now);
            // <Pallet<T>>::chill_controller(controller);

            // make sure to disable validator till the end of this session
            // if SessionInterface::T::disable_validator(controller).unwrap_or(false) {
...
fn kick_out_if_recent<T: Config>(params: SlashParams<T>) {
    ...
    if spans.era_span(params.slash_session).map(|s| s.index) == Some(spans.span_index()) {
        ...
        // <Pallet<T>>::chill_stash(params.controller);

        // make sure to disable validator till the end of this session
        if T::SessionInterface::disable_validator(params.controller).unwrap_or(false) {
            // if SessionInterface::T::disable_validator(params.controller).unwrap_or(false) {
            // if SessionInterface::<T::AccountId>::disable_validator(params.controller).unwrap_or(false) {
            // if <SessionInterface<T::AccountId>>::disable_validator(params.controller).unwrap_or(false) {
            // force a new era, to select a new validator set
            // <Pallet<T>>::ensure_new_era()
        }
...
// apply the slash to a controller account, deducting any missing funds from the reward
