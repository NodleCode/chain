...
use crate::mock::{
	bond_nominator, bond_validator, events, last_event, set_author, start_session,
	on_offence_in_session, balances, on_offence_now, is_disabled, ExistentialDeposit, Balance,
	Balances, Event as MetaEvent, ExtBuilder, NodleStaking, Origin, Session, System, Test,
};
use frame_support::{
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				...
				DispatchError::Module { index: 3, error: 3, message: Some("InsufficientBalance") }
			);
...
fn validator_activate_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(NodleStaking::validator_state(11).unwrap().state, ValidatorStatus::Active);
...
fn validator_commission() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let mut expected =
				vec![Event::ValidatorChosen(2, 1, 40), Event::NewSession(5, 2, 1, 40)];
			assert_eq!(mock::events(), expected);
...
fn set_invulnerables_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_ok!(NodleStaking::set_invulnerables(Origin::root(), new_set.clone()));
...
fn reward_validator_slashing_validator_does_not_overflow() {
	ExtBuilder::default().num_validators(4).has_stakers(true).build_and_execute(|| {
		...
		assert_eq!(
			...
			ValidatorSnapshot {
				...
				nominators: vec![
					Bond { owner: 201, amount: 36893488147419103229 },
					Bond { owner: 301, amount: 36893488147419103229 },
				],
		...
		on_offence_in_session(
			&[OffenceDetails { offender: (81, NodleStaking::at_stake(4, 81)), reporters: vec![] }],
...
fn nominators_also_get_slashed_pro_rata() {
	ExtBuilder::default().build_and_execute(|| {
		...
		tst_log!(trace, "[{:#?}]=> Acc-11 is_disabled[{:#?}]", line!(), mock::is_disabled(11));
		...
		tst_log!(trace, "[{:#?}]=> [{:#?}] | [{:#?}]", line!(), validator_share, nominator_share,);
		...
		tst_log!(trace, "[{:#?}]=> Acc-11 is is_disabled[{:#?}]", line!(), mock::is_disabled(11));
...
fn slashing_performed_according_exposure() {
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		on_offence_now(
			&[OffenceDetails {
				offender: (11, ValidatorSnapshot { total: 500, bond: 500, nominators: vec![] }),
				reporters: vec![],
			}],
			&[Perbill::from_percent(50)],
		);
		...
		assert_eq!(Balances::free_balance(11), validator_balance - 250);
	});
}
...
fn slash_in_old_span_does_not_deselect() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_ok!(NodleStaking::validator_activate(Origin::signed(11)));
		assert_eq!(NodleStaking::validator_state(11).unwrap().state, ValidatorStatus::Active);
		assert_eq!(mock::last_event(), MetaEvent::nodle_staking(Event::ValidatorActive(2, 11)));
		...
		tst_log!(debug, "[{:#?}]=> - {:#?}", line!(), NodleStaking::bonded_sessions());
...
fn subsequent_reports_in_same_span_pay_out_less() {
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		tst_log!(debug, "[{:#?}]=> Acc-1[{:#?}]", line!(), Balances::free_balance(1));
		...
		let mut new1 =
			vec![Event::Slash(11, 200), Event::Slash(101, 100), Event::PayReporterReward(1, 15)];
		expected.append(&mut new1);
		assert_eq!(events(), expected);
		...
		let mut new2 =
			vec![Event::Slash(11, 50), Event::Slash(101, 25), Event::PayReporterReward(1, 10)];
		expected.append(&mut new2);
		assert_eq!(events(), expected);
...
fn invulnerables_are_not_slashed() {
	...
	ExtBuilder::default().invulnerables(vec![11]).build_and_execute(|| {
		...
		let exposure = NodleStaking::at_stake(NodleStaking::active_session(), 21);
		let initial_balance = exposure.bond;
		let nominator_balances: Vec<_> = exposure.nominators.iter().map(|nom| nom.amount).collect();
		...
		assert_eq!(Balances::free_balance(21), 1000 - (2 * initial_balance / 10));

		// ensure that nominators were slashed as well.
		for (initial_balance, nominator) in nominator_balances.into_iter().zip(exposure.nominators)
		{
			assert_eq!(
				Balances::free_balance(&nominator.owner),
				initial_balance - (2 * nominator.amount / 10),
			);
		}
	});
}
...
fn only_slash_for_max_in_session() {
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		// The validator has been slashed and has been deactivated.
...
fn garbage_collection_after_slashing() {
	...
	ExtBuilder::default().existential_deposit(2).build_and_execute(|| {
		...
		assert_eq!(balances(&11).0, validator_balance - (validator_stake / 10));
		assert_eq!(balances(&101).0, nominator_balance - (nominator_stake / 10));

		assert!(<SlashingSpans<Test>>::get(&11).is_some());

		assert_eq!(<SpanSlash<Test>>::get(&(11, 0)).amount_slashed(), &(validator_stake / 10));
		assert_eq!(<SpanSlash<Test>>::get(&(101, 0)).amount_slashed(), &(nominator_stake / 10));
		...
		assert_eq!(balances(&11).0, ExistentialDeposit::get());
...
fn slash_kicks_validators_not_nominators_and_activate_validator_to_rejoin_pool() {
	ExtBuilder::default().build_and_execute(|| {
		...
		// post-slash balance
		assert_eq!(mock::balances(&11), (900, 1000));
		assert_eq!(mock::balances(&101), (1450, 500));
