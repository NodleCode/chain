...
fn multiple_nominations() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(NodleStaking::nominator_state(7).unwrap().nominations.0.len(), 2usize);
			...
			assert_eq!(
				...
				vec![
					...
					Bond { owner: 8, amount: 10 },
					Bond { owner: 9, amount: 10 },
				],
			);
			...
			assert_eq!(NodleStaking::nominator_state(6).unwrap().nominations.0.len(), 3usize);
			assert_eq!(NodleStaking::nominator_state(7).unwrap().nominations.0.len(), 1usize);
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				...
				OrderedSet::from([Bond { owner: 1, amount: 10 }, Bond { owner: 2, amount: 10 },].to_vec()),
			);
			...
			assert_eq!(
				NodleStaking::nominator_state(6).unwrap().nominations,
				OrderedSet::from([Bond { owner: 1, amount: 20 }].to_vec()),
			);
			assert_eq!(NodleStaking::nominator_state(6).unwrap().total, 20);
			assert_eq!(NodleStaking::nominator_state(6).unwrap().active_bond, 20);
			assert_eq!(NodleStaking::total(), 150);
			...
			assert_eq!(
				...
				OrderedSet::from(
					[
						Bond { owner: 1, amount: 20 },
						Bond { owner: 4, amount: 15 },
						Bond { owner: 5, amount: 20 },
					]
					.to_vec()
				),
			);
...
fn reconciliation_basics_works() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				NodleStaking::nominator_state(6).unwrap().nominations,
				OrderedSet::from([Bond { owner: 1, amount: 10 }, Bond { owner: 2, amount: 40 },].to_vec()),
			);
			...
			let s1_min_nomination_chill_threshold = NodleStaking::staking_min_nomination_chill_threshold();
			...
			assert_eq!(
				NodleStaking::nominator_state(6).unwrap().nominations,
				OrderedSet::from([Bond { owner: 2, amount: 40 },].to_vec()),
			);
			assert_eq!(NodleStaking::nominator_state(6).unwrap().total, 50);
			assert_eq!(NodleStaking::nominator_state(6).unwrap().active_bond, 40);
...
fn unfreeze_bond_arg_flag_works() {
	ExtBuilder::default()
		...
		.with_validators(vec![(1, 20), (2, 20), (3, 20), (4, 20), (5, 10)])
		.with_nominators(vec![(6, 1, 10), (7, 1, 10), (8, 2, 10), (9, 2, 10), (10, 1, 25)])
		.tst_staking_build()
		.execute_with(|| {
			mock::start_active_session(4);
			...
			assert_eq!(
				...
				OrderedSet::from(
					[
						Bond { owner: 6, amount: 10 },
						Bond { owner: 7, amount: 10 },
						Bond { owner: 10, amount: 25 },
					]
					.to_vec()
				),
			);
			...
			assert_eq!(
				...
				OrderedSet::from([Bond { owner: 6, amount: 20 },].to_vec()),
			);

			assert_eq!(
				NodleStaking::nominator_state(6).unwrap().nominations,
				OrderedSet::from([Bond { owner: 1, amount: 10 }, Bond { owner: 3, amount: 20 },].to_vec()),
			);
			assert_eq!(NodleStaking::nominator_state(6).unwrap().total, 30);
			...
			assert_eq!(
				NodleStaking::nominator_state(8).unwrap().nominations,
				OrderedSet::from([Bond { owner: 2, amount: 10 }, Bond { owner: 4, amount: 20 },].to_vec()),
			);
			...
			assert_eq!(
				NodleStaking::validator_state(1).unwrap().nominators,
				OrderedSet::from([Bond { owner: 10, amount: 25 },].to_vec()),
			);

			assert_eq!(
				NodleStaking::nominator_state(6).unwrap().nominations,
				OrderedSet::from([Bond { owner: 3, amount: 20 },].to_vec()),
			);
			assert_eq!(NodleStaking::nominator_state(6).unwrap().total, 30);
			assert_eq!(NodleStaking::nominator_state(6).unwrap().active_bond, 20);
			assert_eq!(NodleStaking::nominator_state(6).unwrap().frozen_bond, 10);
			...
			assert_eq!(
				NodleStaking::nominator_state(8).unwrap().nominations,
				OrderedSet::from([Bond { owner: 4, amount: 20 },].to_vec()),
			);
			assert_eq!(NodleStaking::nominator_state(8).unwrap().total, 30);
			...
			assert_eq!(
				NodleStaking::validator_state(1).unwrap().nominators,
				OrderedSet::from([Bond { owner: 6, amount: 30 }, Bond { owner: 10, amount: 25 }].to_vec()),
			);
...
fn set_total_validator_per_round_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		let mut expected = vec![Event::TotalSelectedSet(old_total_selected, new_total_selected)];
		assert_eq!(events(), expected);
		...
		let mut new1 = vec![Event::TotalSelectedSet(old_total_selected * 4u32, new_total_selected)];
		expected.append(&mut new1);
		assert_eq!(events(), expected);
...
fn set_staking_limits_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		let old_min_nomination_chill_threshold = NodleStaking::staking_min_nomination_chill_threshold();
		...
		let new_min_nomination_total_bond = old_min_nomination_total_bond.saturating_mul(4u32.into());
		let new_min_nomination_chill_threshold = old_min_nomination_chill_threshold.saturating_mul(4u32.into());
		...
		assert_eq!(NodleStaking::staking_min_stake_session_selection(), new_min_stake);
		assert_eq!(NodleStaking::staking_min_validator_bond(), new_min_validator_bond);
		assert_eq!(
		...
		let new2_min_nomination_total_bond = old_min_nomination_total_bond.saturating_mul(2u32.into());
		...
		assert_eq!(NodleStaking::staking_min_stake_session_selection(), new2_min_stake);
		assert_eq!(NodleStaking::staking_min_validator_bond(), new2_min_validator_bond);
		assert_eq!(
			NodleStaking::staking_min_nominator_total_bond(),
			new2_min_nomination_total_bond
		);
...
fn reward_from_authorship_event_handler_works() {
	ExtBuilder::default().num_validators(4).build_and_execute(|| {
		...
		assert_eq!(NodleStaking::awarded_pts(NodleStaking::active_session(), 11), 25);

		// 21 is rewarded as an uncle producer
		assert_eq!(NodleStaking::awarded_pts(NodleStaking::active_session(), 21), 1);
...
fn add_reward_points_fns_works() {
	ExtBuilder::default().num_validators(4).build_and_execute(|| {
		...
		assert_eq!(NodleStaking::awarded_pts(NodleStaking::active_session(), 11), 4);

		assert_eq!(NodleStaking::awarded_pts(NodleStaking::active_session(), 21), 2);

		assert_eq!(NodleStaking::awarded_pts(NodleStaking::active_session(), 41), 2);
	...
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			...
			let mut expected = vec![
				...
				Event::JoinedValidatorPool(81, 36893488147419103230, 36893488147419106730),
				Event::ValidatorChosen(4, 11, 1500),
				Event::ValidatorChosen(4, 21, 1000),
				Event::ValidatorChosen(4, 41, 1000),
				Event::ValidatorChosen(4, 81, 36893488147419103230),
				Event::NewSession(15, 4, 4, 36893488147419106730),
				Event::StakeReward(81, 36893488147419103230),
				Event::ValidatorChosen(5, 11, 1500),
				Event::ValidatorChosen(5, 21, 1000),
				Event::ValidatorChosen(5, 41, 1000),
				Event::ValidatorChosen(5, 81, 36893488147419103230),
				Event::NewSession(20, 5, 4, 36893488147419106730),
			];
			assert_eq!(events(), expected);
...
fn nominators_also_get_slashed_pro_rata() {
	ExtBuilder::default().num_validators(4).build_and_execute(|| {
		...
		assert_eq!(mock::balances(&101), (1975, nominator_stake - nominator_share));
		assert_eq!(mock::balances(&11), (1950, validator_stake - validator_share),);
...
fn slashing_performed_according_exposure() {
	...
	ExtBuilder::default().num_validators(4).build_and_execute(|| {
		...
		assert_eq!(NodleStaking::at_stake(NodleStaking::active_session(), 11).bond, 1000);
...
fn slash_in_old_span_does_not_deselect() {
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			...
			on_offence_now(
				&[OffenceDetails {
					offender: (11, NodleStaking::at_stake(NodleStaking::active_session(), 11)),
					reporters: vec![],
				}],
...
fn invulnerables_are_not_slashed() {
	...
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			...
			let validator21_nominator_initial_bond: Vec<_> =
				valid21_exposure.nominators.iter().map(|nom| nom.amount).collect();
			...
			on_offence_now(
				&[
					...
					OffenceDetails {
						offender: (21, NodleStaking::at_stake(NodleStaking::active_session(), 21)),
						reporters: vec![],
					},
				],
			...
			assert_eq!(
				mock::balances(&21),
				(1800, validator21_initial_bond - (2 * validator21_initial_bond / 10))
			);
...
fn slashing_nominators_by_span_max() {
	ExtBuilder::default().num_validators(4).build_and_execute(|| {
		...
		let exposure_11 = NodleStaking::at_stake(NodleStaking::active_session(), 11);
		let exposure_21 = NodleStaking::at_stake(NodleStaking::active_session(), 21);
		let nominated_value_11 = exposure_11.nominators.iter().find(|o| o.owner == 101).unwrap().amount;
		let nominated_value_21 = exposure_21.nominators.iter().find(|o| o.owner == 101).unwrap().amount;
		...
		assert_eq!(mock::balances(&101), (1950, 1000 - nominator101_prev_slashed_val,));
		...
		assert_eq!(mock::balances(&101), (1850, 950 - nominator101_prev_slashed_val));
...
fn remove_multi_deferred() {
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			...
			on_offence_now(
				&[OffenceDetails {
					offender: (41, NodleStaking::at_stake(NodleStaking::active_session(), 11)),
					reporters: vec![],
				}],
				&[Perbill::from_percent(25)],
				DisableStrategy::Always,
			...
			on_offence_now(
				&[OffenceDetails {
					offender: (69, NodleStaking::at_stake(NodleStaking::active_session(), 11)),
					reporters: vec![],
				}],
				&[Perbill::from_percent(25)],
				DisableStrategy::Always,
