...
pub mod pallet {
    ...
    pub trait Config: frame_system::Config + pallet_emergency_shutdown::Config {
        ...
        type ProtocolFeeReceiver: WithAccountId<<Self as system::Config>::AccountId>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn allocate(
            origin: OriginFor<T>,
            to: <T as system::Config>::AccountId,
            amount: BalanceOf<T>,
            proof: Vec<u8>,
    ...
    #[pallet::metadata(<T as system::Config>::AccountId = "AccountId", BalanceOf<T> = "Balance")]
    ...
    pub type Oracles<T: Config> =
        StorageValue<_, Vec<<T as system::Config>::AccountId>, ValueQuery>;
...
impl<T: Config> Pallet<T> {
    pub fn is_oracle(who: <T as system::Config>::AccountId) -> bool {
        Self::oracles().contains(&who)
    }

    fn ensure_oracle(origin: <T as system::Config>::Origin) -> DispatchResult {
...
impl<T: Config> ChangeMembers<<T as system::Config>::AccountId> for Pallet<T> {
    fn change_members_sorted(
        _incoming: &[<T as system::Config>::AccountId],
        _outgoing: &[<T as system::Config>::AccountId],
        new: &[<T as system::Config>::AccountId],
    ) {
        <Oracles<T>>::put(new);
    }
}

impl<T: Config> InitializeMembers<<T as system::Config>::AccountId> for Pallet<T> {
    fn initialize_members(init: &[<T as system::Config>::AccountId]) {
        <Oracles<T>>::put(init);
    }
}
